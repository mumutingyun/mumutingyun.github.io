...
Hexo 的工作流程是**先编译，后发布**。网站访问者看到的是编译后的 HTML，不是你的 Markdown 源文件：

```
源文件 (source/_posts/xxx.md)
    ↓  hexo generate（生成）
静态网页 (public/xxx/index.html)
    ↓  hexo deploy（部署）
网站 (https://mumutingyun.netlify.app)
```

- 你改/删了源文件，但 `public/` 目录里的旧 HTML 还在
- `hexo generate` 会根据当前源文件重新生成 `public/`
- `hexo deploy` 把更新后的文件推送到线上

> 📱 **例外**：手机 CMS 点"发布"会自动完成生成+部署，无需手动操作。
...
**方式 2：VS Code 中删除**
在左侧文件资源管理器找到 `source/_posts/`，右键文件 → Delete，然后终端执行 `hexo g -d`。

**方式 3：手机 CMS 删除**
登录 `https://mumutingyun.netlify.app/admin/`，进入文章后点删除（自动部署，无需手动 `hexo g -d`）。
...
| 场景 | 操作方式 |
|------|---------|
| 💻 电脑写文章 | `hexo new "标题"` → 编辑 `.md` → `hexo g -d` 发布 |
| 📱 手机写文章 | 浏览器打开 `/admin/` → GitHub 登录 → CMS 编辑 → 点"发布" |
| ✏️ 重命名文章 | `mv` 改名 → `code` 修改 title → `hexo g -d` |
| 🗑️ 删除文章 | `rm` 删除 → `hexo g -d`（CMS 删除自动部署） |
